...
const render_js_1 = __importStar(require("./render.js"));
const scrub = (object) => {
...
describe('render', () => {
    describe('collapseWhitespace', () => {
        it('Should reduce all continuous whitespace to a single space', () => {
            expect((0, render_js_1.collapseWhitespace)('\n\n foo  \t  bar  ')).toBe(' foo bar ');
        });
    });
    describe('bucketElements', () => {
      ...
      `;
            ...
            const bucketed = (0, render_js_1.bucketElements)(elements, true);
      ...
      `;
            ...
            const bucketed = (0, render_js_1.bucketElements)(elements, false);
            expect(bucketed).toEqual([
                { hasBlock: true, content: [blockElement, blockElement] },
    ...
    describe('hasBlockContent', () => {
        it('Should return false for string elements', () => {
            expect((0, render_js_1.hasBlockContent)('')).toBe(false);
        });
        it('Should return false for inline elements', () => {
            expect((0, render_js_1.hasBlockContent)(inlineElement)).toBe(false);
        });
        it('Should return true for block elements', () => {
            expect((0, render_js_1.hasBlockContent)(blockElement)).toBe(true);
        });
        it('Should return true for inline elements with block content', () => {
            ...
            expect((0, render_js_1.hasBlockContent)(inlineElementWithBlockContent)).toBe(true);
        });
        it('Should return true for anchor elements with block content', () => {
            ...
            expect((0, render_js_1.hasBlockContent)(anchorWithBlockContent)).toBe(true);
        });
        it('Should return false for anchor elements with inline content', () => {
            ...
            expect((0, render_js_1.hasBlockContent)(anchorWithInlineContent)).toBe(false);
        });
        it('Should return false for inline elements with inline content', () => {
            ...
            expect((0, render_js_1.hasBlockContent)(inlineElementWithBlockContent)).toBe(false);
        });
    });
    describe('renderHtml', () => {
        it('Should correctly merge styles', () => {
            const result1 = (0, render_js_1.default)('<p></p>', {
                style: {
                    color: 'blue',
                },
            });
            ...
            const result2 = (0, render_js_1.default)('<p></p>', {
                style: [
                    {
                        color: 'blue',
                    },
        ...
        it('Should be able to render external CSS', () => __awaiter(void 0, void 0, void 0, function* () {
            const html = `<html>
  <head>
    <link
      rel="stylesheet"
  ...
  <body>
    <div></div>
            ...
            const rendered = (0, render_js_1.default)(html, { stylesheet: stylesheets });
            console.log(rendered);
        }));
        it('Should use a custom renderer', () => {
            const foo = jest.fn();
            const html = '<foo>Custom Content</foo>';
            const rootView = (0, render_js_1.default)(html, {
                renderers: {
                    foo,
                },
            });
        ...
        it('Should render a link with text inside it', () => {
            ...
            expect(a.type).toBe(renderers_js_1.default.a);
        ...
        it('Should render inline elements with proper text wrapper', () => {
            ...
            expect(p.type).toBe(renderers_js_1.renderBlock);
        ...
        it('Should render keep spaces around custom elements', () => {
            ...
            const rootView = (0, render_js_1.default)(html, {
                renderers: {
                    customelement: () => React.createElement(renderer_1.View, null),
                },
            });
        ...
        it('Should render span as block style suggests it is block', () => {
            ...
            expect(blockSpan.type).toBe(renderers_js_1.renderBlock);
...
`;
            const rootView = (0, render_js_1.default)(content);
            expect(rootView.type).toBe(renderer_1.View);
...
`;
            ...
            expect(table.type).toBe(renderers_js_1.default.table);
            ...
            const renderedRow1Col1 = (0, renderers_js_1.renderCell)(row1Col1.props);
            console.log('renderedRow1Col1', JSON.stringify(renderedRow1Col1, null, 2));
            expect(renderedRow1Col1 === null || renderedRow1Col1 === void 0 ? void 0 : renderedRow1Col1.props.style[0].width).toBe('33.33333%');
            ...
            const renderedRow2Col1 = (0, renderers_js_1.renderCell)(row2Col1.props);
            console.log('renderedRow2Col1', JSON.stringify(renderedRow2Col1, null, 2));
            expect(renderedRow2Col1 === null || renderedRow2Col1 === void 0 ? void 0 : renderedRow2Col1.props.style[0].width).toBe('66.66667%');
            const row2Col2 = row2.props.children[1];
            expect(row2Col2.props.element.tag).toBe('td');
            const renderedRow2Col2 = (0, renderers_js_1.renderCell)(row2Col2.props);
            console.log('renderedRow2Col2', JSON.stringify(renderedRow2Col2, null, 2));
            expect(renderedRow2Col2 === null || renderedRow2Col2 === void 0 ? void 0 : renderedRow2Col2.props.style[0].width).toBe('33.33333%');
            expect(renderedRow2Col2 === null || renderedRow2Col2 === void 0 ? void 0 : renderedRow2Col2.props.style[2].width).toBe('10%');
        });
    ...
    <ul>
        <li>
          ...
          <ul><li>Second level</li></ul>
            ...
            expect(html.type).toBe(renderers_js_1.renderPassThrough);
            expect(html.props.children.length).toBe(2);
            const style = html.props.children[0];
            expect(style.type).toBe(renderers_js_1.renderNoop);
            expect(style.props.element.tag).toBe('style');
            const body = html.props.children[1];
            expect(body.type).toBe(renderers_js_1.renderBlock);
            ...
            expect(image.type).toBe(renderers_js_1.default.img);
            expect(image.props.element.tag).toBe('img');
            const ul = body.props.children[1];
            expect(ul.type).toBe(renderers_js_1.renderBlock);
            ...
            //   }
            // });
            const document = (React.createElement(renderer_1.Document, null,
                React.createElement(renderer_1.Page, { size: "LETTER" },
                    React.createElement(React.Fragment, null, rootView))));
            const pdfString = yield (0, renderer_1.renderToString)(document);
            // console.log(pdfString);
        }));
    });
    it('Should render newline as space between text elements', () => __awaiter(void 0, void 0, void 0, function* () {
...
<span> world</span></p>`;
    ...
    describe('GitHub issue #117 - Whitespace preservation', () => {
        it('Should preserve spaces between inline elements', () => {
            ...
            expect(textContent.type).toBe(renderer_1.Text);
            ...
            const boldElement = children.find((child) => child.props && child.props.element && child.props.element.tag === 'b');
            ...
            const plainText = children.find((child) => typeof child === 'string');
            expect(plainText).toBe('b');
        });
        it('Should preserve space content in strong elements', () => {
            // Test that spaces inside elements are preserved
            const html = '<p>Press <span style="color: rgb(250, 176, 5);"><strong>YELLOW</strong></span><span style="color: rgb(233, 245, 12);"><strong> </strong></span><strong>SAMPLE </strong> button again</p>';
            const rootView = (0, render_js_1.default)(html);
            expect(rootView.type).toBe(renderer_1.View);
            const p = rootView.props.children;
            expect(p.props.element.tag).toBe('p');
